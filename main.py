...
def load_encodings(folder_name: str):
    try:
        ...
        logger.warning(f"Failed to load encodings for folder {folder_name}: {e}")
        return None
    ...
    try:
        ...
        for item in drive_images:
            try:
                ...
                encs = face_recognition.face_encodings(drive_image)
                if encs:
        ...
        with zipfile.ZipFile(zip_stream, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
            for file_id in image_ids:
                try:
                    ...
                    print(f"Failed to fetch file with ID {file_id}: {str(e)}")
                    continue
